require('global')
local fighterModule = require('fighter')
Fighter = fighterModule.Fighter
Actions = fighterModule.Actions
Input = require('input')
HitBoxType = fighterModule.HitBoxType
...
local warriorSheet, wizardSheet, victoryImage
local swordSound, magicSound, victorySound, readySound, fightSound, musicBG
local countFont, scoreFont
...
function love.load()
	...
	countFont = lettersFont
	scoreFont = lettersFont
...
function checkPause()
	if Input.once(1,BTN_START) or Input.once(2,BTN_START) then
		PAUSE = not PAUSE
		if PAUSE then musicBG:stop() else musicBG:play() end
	end
end
...
function love.draw()
	...
	drawHealthBar(fighter2.health,(SCREEN_WIDTH/2)+10,20)
	fighter1:draw()
	fighter2:draw()
