...
local kbdmap = {
	's','a','rshift','return','up','down','left','right','d','w','q','e',
}

local padmap = {
	'a','x','back','start','dpup','dpdown','dpleft','dpright','b','y','leftshoulder','rightshoulder',
}

local axismap = { 'leftx','lefty','rightx','righty' }
...
return {
		...
		for pad = 1,2 do
			for btn = 1,12 do
				if love.joystick.isDown(pad, btn) then
					state[pad][btn] = (state[pad][btn] or 0) + 1
				else
					state[pad][btn] = 0
				end
			end
			local lx = love.joystick.getAxis(pad, AXIS_LEFT_X)
			local ly = love.joystick.getAxis(pad, AXIS_LEFT_Y)
			state[pad][BTN_L_WEST] = (lx<-0.5 and (state[pad][BTN_L_WEST] or 0)+1) or 0
			state[pad][BTN_L_EAST] = (lx>0.5 and (state[pad][BTN_L_EAST] or 0)+1) or 0
			state[pad][BTN_L_NORTH] = (ly<-0.5 and (state[pad][BTN_L_NORTH] or 0)+1) or 0
			state[pad][BTN_L_SOUTH] = (ly>0.5 and (state[pad][BTN_L_SOUTH] or 0)+1) or 0
		end
	end,

	isDown = function(pad, btn)
		return not daft and (state[pad][btn] or 0) > 0
	end,

	once = function(pad, btn)
		if daft then return false end
		if (state[pad][btn] or 0) == 1 then
			state[pad][btn] = 2
			return true
		end
		return false
	...
	withCooldown = function(pad, btn)
		return not daft and (state[pad][btn] or 0)%32==1
	end,
